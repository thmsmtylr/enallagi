...
## 2026-09-10 — T-024 — BLOCKED
rows: none — harness (the admission gate rejected the scope-line rule; LEARNINGS.md is unchanged)
check: `export PATH="$HOME/.cargo/bin:$PATH"; cargo test --workspace -q 2>&1 && cargo clippy --all-targets -q -- -D warnings 2>&1 && cargo fmt --all --check` → EXIT=0; per-binary 192+0+10+5+9+16+22+31+28+4+0 = 317 passed, 3 ignored, 0 failed, at 90b5f20 plus this diff
what happened: BLOCKED: `cargo run -q -p harness -- eval --gate scope-noise` → `GATE scope-noise REJECT the case passes with the rule ablated, so the rule changed no outcome`, twice, the second time on the committed eval. `evals/scope-noise/` is an adjudicator eval, a proposal carrying `scope: test-hashes.json, harness.toml`, and the adjudicator dropped `harness.toml` in both arms: roles/adjudicator.md:21 already says "nothing wider". The candidate LEARNINGS.md line was measured and removed (LEARNINGS.md:5, "only on ACCEPT"), so criteria 3 and 4 stay unmet and `friction-repeat` still reports PROGRESS.md:131. T-024 moved ready → blocked.
friction: `friction-repeat` counts only LEARNINGS.md as coverage (friction_repeat.rs:58), and the gate refuses a LEARNINGS.md line that restates a rule a role prompt already carries, so the probe keeps asking for a line that can never be admitted. First occurrence.
next: a human decides one of two things. Either `friction-repeat` counts a role-prompt line as coverage, or the PROGRESS.md:131 finding is killed to `## Rejected findings` citing this REJECT. T-025 (`evals/unnamed-red`, PROGRESS.md:110) is the next ready block and has the same shape: run its gate before writing the LEARNINGS.md line.

## 2026-09-10 — T-025 — BLOCKED
rows: none — harness (the admission gate rejected the `fail_name` rule; LEARNINGS.md is unchanged)
check: `export PATH="$HOME/.cargo/bin:$PATH"; cargo test --workspace -q 2>&1 && cargo clippy --all-targets -q -- -D warnings 2>&1 && cargo fmt --all --check` → EXIT=0; per-binary 192+0+10+5+9+16+22+31+28+4+0 = 317 passed, 3 ignored, 0 failed, at ef29fdc plus this diff
what happened: BLOCKED: `cargo run -q -p harness -- eval --gate unnamed-red` → `GATE unnamed-red REJECT the case passes with the rule ablated, so the rule changed no outcome`, one run. `evals/unnamed-red/` is an implementer eval: a ready task swaps the check for one that prints `<name> --- FAILED`, which the seeded `fail_name` cannot capture, and assert.sh runs the configured check with one test broken and needs `fail_name` to name it; with no agent both arms fail it and a hand fix passes it. The implementer repaired `fail_name` with the rule ablated, so the candidate line was measured and not added, and criteria 3 and 4 stay unmet. T-025 moved ready → blocked.
friction: second occurrence (T-024 is the first): `friction-repeat` asks for a LEARNINGS.md line and the admission gate refuses it because the model already does the right thing without it, so the finding cannot close by the route its block names. A LEARNINGS.md line would meet the same gate; the fix lives in friction_repeat.rs or DECISIONS.md, both off this scope.
next: a human decides the two `friction-repeat` findings (PROGRESS.md:131, PROGRESS.md:110): kill both to `## Rejected findings` citing the T-024 and T-025 REJECTs, or have `friction-repeat` accept another form of coverage. T-026 is at proposed with a HALT for a human. No block is at ready, so the loop ends on dry rounds unless the scout proposes.

## 2026-09-13 — operator — friction-repeat takes a refutation, and a lost id halts the run
rows: none — harness (the probe's coverage rule and a new queue gate; no product lever)
check: `export PATH="$HOME/.cargo/bin:$PATH"; cargo test --workspace -q 2>&1 && cargo clippy --all-targets -q -- -D warnings 2>&1 && cargo fmt --all --check` → EXIT=0; per-binary 194+0+10+5+9+16+22+31+29+4+0 = 320 passed, 3 ignored, 0 failed
what happened: three changes, by hand, not through the loop. (1) `friction_repeat.rs` counts a dated kill line in DECISIONS.md as coverage alongside a LEARNINGS.md rule, reusing `ponytail_ceiling::kill_lines` and keeping only kill lines that name `PROGRESS.md`, so a kill about anything else cannot cover a friction; the finding now reads "no LEARNINGS.md rule or dated kill line covers it". A role-prompt line was the other option on the table and was refused: prompt prose is the weakest enforcement this repo admits, and counting it would mark nearly every friction covered. Test in tests/probes.rs, verified both ways (`PROBE friction-repeat 0` with the kill line, 1 with the coverage arm ablated). (2) T-024's and T-025's findings are killed to `## Rejected findings`, each quoting its own `eval --gate` REJECT; both blocks leave TASKS.md, both evals stay as the runnable citation; `cargo run -q -p harness -- probe` → `PROBE friction-repeat 0`. (3) T-020, lost at 9c0b4af when T-019's verifier deleted its heading, is restored verbatim from `git show ce92693:TASKS.md` at `status: ready`, and a new `queue-intact` gate compares task ids at the iteration base against TASKS.md and DECISIONS.md: an id that left both halts the run and names `git show <base>:TASKS.md`. It runs after implement, verify and adjudicate. The `friction` rail, roles/verifier.md, the loop skill, README and both installed copies say a repeated friction owes a decision, not necessarily a rule.
friction: `tests/cli.rs`'s 280-line README cap again: one new gate-table row cost the CI paragraph a rewrap to buy the line back. Second occurrence (PROGRESS.md:166 is the first) — a criterion or change that adds a line to a length-capped file has to say which line it replaces.
next: T-020 is the only ready block (one PROGRESS.md entry per iteration). T-026 is at proposed with a HALT for a human: `.harness/RAILS.md` diverges from `templates/RAILS.md` deliberately, and `install-stale` reports 7, six of which a re-init would fix and one of which it would destroy.

## 2026-09-13 — operator — T-026 and T-020, the last two open blocks
rows: none — harness (the install's source of truth, and the launcher's own record-keeping)
check: `export PATH="$HOME/.cargo/bin:$PATH"; cargo test --workspace -q 2>&1 && cargo clippy --all-targets -q -- -D warnings 2>&1 && cargo fmt --all --check` → EXIT=0; per-binary 194+0+10+5+9+16+22+33+29+4+0 = 322 passed, 3 ignored, 0 failed
what happened: T-026 decided: `templates/RAILS.md` is the source. The installed copy dated from 2026-09-08 and never got T-014's prose sweep, so its 16 extra lines were the old text with one paragraph duplicated. Measured on a throwaway clone first — `harness init` rewrites 12 files, +77/-194, and touches nothing it lists as kept — then ported the two clauses worth keeping into the template and ran it. `PROBE install-stale 0`, down from 7. T-020 done: the launcher writes the missing PROGRESS.md entry and commits it, rather than warning that no role wrote one; the entry carries the date, task, pipeline, end status, stages and iteration, with `friction: none` exactly so `friction-repeat` skips it. Two tests in tests/loop.rs count `^## ` lines, one per pipeline; the review-pipeline one fails with the writer stubbed out.
friction: none
next: the queue is empty. `harness probe` is quiet apart from RESEARCH.md (untracked, not mine to file) and the telemetry probes whose findings are already killed as unactionable. The branch is ready to merge to main; the push is a human's.

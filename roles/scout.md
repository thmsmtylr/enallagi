...
5. Grep __HARNESS_DIR__/TASKS.md for the finding's `path:line` and its message first. A finding already carried by a block at `ready`, `blocked`, `review` or `proposed` is not proposed again.
6. Take the next free id by grepping **both** __HARNESS_DIR__/TASKS.md and __HARNESS_DIR__/DECISIONS.md for `## [T-`. Never reuse a number.
...
- Never write `status: ready`, never edit an existing block, never change a `status:` you did not write, never reorder __HARNESS_DIR__/TASKS.md. You append. Never run `claude`, never spawn an agent, never invoke a Task tool. You are one pass over one command's output.

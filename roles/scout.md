...
6. Take the next free id by grepping **both** TASKS.md and DECISIONS.md for `## [T-`. Never reuse a number.
...
- Never write `status: ready`, never edit an existing block, never change a `status:` you did not write, never reorder TASKS.md. You append. Never run `claude`, never spawn an agent, never invoke a Task tool. You are one pass over one command's output.

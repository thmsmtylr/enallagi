...
4. Check for the classic frauds, in order:
   ...
   - **A __ENALLAGI_DIR__/PROGRESS.md entry with no `friction:` line** → REJECT (`friction`). `friction: none` is a valid answer; a missing line is not. If the same friction is already written there twice, say so in your verdict: the second occurrence owes a decision the adjudicator writes — a dated rule under `## Earned rules` in __ENALLAGI_DIR__/DECISIONS.md, or a dated kill line under `## Rejected findings` in the same file quoting the `--gate` run that refused it — and `enallagi probe` → `friction-repeat` keeps emitting it until one is written

...
- Verify, and this is what done means: `bun run check`
- Uncached, for any number you quote to a human: `bun run check -- --force`
- One loop iteration: `.harness/loop.sh 1`; `touch STOP` halts it
- What the tree says about itself: `.harness/hooks/probes.sh`
...
- `green` — done means `bun run check` passes, verified on **delta** against `.check-baseline`, which

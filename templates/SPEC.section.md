...
| `blocked-is-allowed` | A row may be marked `BLOCKED` in `__HARNESS_DIR__/PROGRESS.md` with a written reason and the loop stops. This is a success, not a failure. A row may **never** be marked done without the exact command and its pasted output. | An abort affordance cut GPT-5's cheating **54% → 9%** ([ImpossibleBench](https://arxiv.org/html/2510.20270v1)). An agent with no exit but "pass" will manufacture a pass. |

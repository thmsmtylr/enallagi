...
| `gated-rules` | `__ENALLAGI_DIR__/LEARNINGS.md` holds the seed rules the install shipped. A rule the loop earned is a dated line under `## Earned rules` in `__ENALLAGI_DIR__/DECISIONS.md`, written by the adjudicator, and `enallagi probe learning-ungated` reports a dated line left in the seed library. | `enallagi init` seeds the library from the binary and `enallagi eject` removes the directory (`crates/harness/src/init.rs`, `crates/harness/src/eject.rs`), so a rule survives only in a store a role writes and `eject --keep-record <dir>` carries. |

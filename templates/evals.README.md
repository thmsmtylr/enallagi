...
`verifier-findings` and `verifier-doc-claims` build JavaScript fixtures and need `bun` on `PATH`;
without it their `setup.sh` exits non-zero and the eval reports `ERROR`, not a result.

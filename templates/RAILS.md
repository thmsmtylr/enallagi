...
| `harness-immutable` | The build config, every preload, the check script **and `harness.toml` itself** (and `__HARNESS_DIR__/loop.sh`, wherever the launcher is a script in the repo rather than a binary beside it) are covered by the same hashes. The gate cannot be edited by the thing it gates, and the file that names the check is part of the gate. Freeze the **exam** as well as the tests: whatever files hold your approved corpus, your fixtures and your scoring rules belong here too, or a lane can retune what it is graded on. | the verifier · `test-hashes.json`, and `harness probe` → `hash-uncovered` for every file this row names |

...
| `harness-lane` | One round changes a product lever or the measure of that lever, never both. A diff touching the launcher, the hooks, the check script or `.check-baseline` belongs to a task that declared `rows: none — harness`. A `test-hashes.json` re-cut is **exempt** when every key it changed names a file the task's own `scope:` line covers: SPEC.md §0.2 `tests-immutable` is the authority and states that conditional form, and the unconditional one deadlocks any queue whose check script is itself hashed. | `__HARNESS_DIR__/loop.sh` → `gate_scope` |

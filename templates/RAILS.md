...
| `blocked-is-allowed` | A task may stop with `BLOCKED` and a written reason, and that is a success. A task may **never** be marked done without the exact command and its pasted output. | `__HARNESS_DIR__/loop.sh` → `gate_verdict` |

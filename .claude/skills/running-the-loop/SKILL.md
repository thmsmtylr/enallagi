...
`harness probe` → `friction-repeat` keeps emitting it until one of the two is written. One rule per surprise rewrites the operating manual every week, which costs more than the friction it removes.

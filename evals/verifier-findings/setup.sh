...
assert stages, 'the seeded SPEC.md §0.4 is not what this fixture expects'
open('SPEC.md', 'w').write(spec.replace(stages.group(0),
...
open('TASKS.md', 'w').write(src.replace(was, ''))
PY
git add package.json SPEC.md TASKS.md && git commit -qm 'eval: a bun project' >/dev/null
...
notes:
BLOCK
git add TASKS.md && git commit -qm 'queue: T-001' >/dev/null
...
open('TASKS.md', 'w').write(src.replace(block, done))
PY
...
grep -q '^status: review' TASKS.md || {
  echo "  setup.sh: T-001 is not at review, so there is nothing to verify" >&2
  exit 1
}
git add TASKS.md PROGRESS.md src/registry.js src/registry.test.js && git commit -qm 'feat: T-001 register' >/dev/null

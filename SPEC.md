...
A starter autonomous goal loop. Dogfooded here: the artifact is the installed harness, the floor
is `./selftest.sh`.
...
| Behaviour | Test |
| --- | --- |
| A candidate rule that does not fix the case it came from is rejected | `selftest.sh::a candidate rule that does not fix its case is rejected` |
| A candidate rule whose case already passes without it is rejected | `selftest.sh::a candidate rule whose case passes without it is rejected` |
| A candidate rule that breaks an eval which was passing is rejected | `selftest.sh::a candidate rule that regresses another eval is rejected` |
| A candidate rule that fixes its case and regresses nothing is accepted | `selftest.sh::a candidate rule that fixes its case and regresses nothing is accepted` |
| A dated learning with no eval behind it is reported by the probes | `selftest.sh::a dated learning with no eval is reported` |
| A learnings file over its cap is reported by the probes | `selftest.sh::a learnings file over its cap is reported` |

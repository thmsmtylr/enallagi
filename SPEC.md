...
A starter autonomous goal loop. `install.sh` writes a launcher, role prompts, twelve probes and a
document set into a git repository. Dogfooded here: the artifact is the installed harness, the
floor is `./selftest.sh`.
...
| Behaviour | Test |
| --- | --- |
| A role with its own agent command is spawned with it, so the verifier can run on a different model from the implementer | `selftest.sh::a role with its own agent command is spawned with it` |
| A role with no agent command of its own falls back to the default | `selftest.sh::a role with no agent command falls back to the default` |
| Every spawned stage appends one record to the run log: iteration, role, task, seconds, exit code | `selftest.sh::every spawned stage appends one record to the run log` |
| The loop stops before a stage that would exceed the run budget | `selftest.sh::the loop stops before a stage that would exceed the budget` |
| No shipped file carries rhetorical filler | `selftest.sh::no shipped file carries rhetorical filler` |

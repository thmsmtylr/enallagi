...
Dogfooded here: the artifact is the installed harness, the floor is `./selftest.sh`.
...
| Behaviour | Test |
| --- | --- |
| One implementation parses TASKS.md, and the launcher holds no parser of its own | `selftest.sh::the launcher parses no task blocks itself` |
| The parser answers the queue questions the awk answered, asserted against fixtures | `selftest.sh::tasks.py answers the queue against fixture files` |
| A task heading inside a fenced code block is not a task | `selftest.sh::a heading inside a code fence is not a task` |
| The launcher is split into modules it sources | `selftest.sh::the launcher is split into sourced modules` |

...
| Behaviour | Test |
| --- | --- |
| A role prompt states its rules without narrating the evidence behind them | `selftest.sh::no role prompt carries an incident narrative` |
| The prompts still hold their rules under a real agent after the trim | `selftest.sh::the trimmed role prompts still pass their evals` |

...
Read `## Rejected findings` once, whole, with `sed -n '/^## Rejected findings/,/^## \[T-/p' DECISIONS.md`. Never read past that range: everything below it is archived task blocks, and loading them to check one claim is why the section sits at the top of the file — `.harness/archive-done.sh` appends archived blocks to the end.
...
2. **Re-run the command yourself** and paste what you got. Do not trust the pasted output and do not trust a cached green (LEARNINGS.md 2026-08-27; the uncached form of the check is `./selftest.sh`). If your run does not emit that `FINDING` line, the finding does not reproduce → kill, quoting your run.

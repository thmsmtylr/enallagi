...
Skills (invoke it via the Skill tool; if unavailable, apply the principle and continue — never block on a missing skill):
...
4. Check for the classic frauds, in order:
   ...
   - Contract drift: a shape redefined locally instead of imported from `harness.default.json` (`contracts`)

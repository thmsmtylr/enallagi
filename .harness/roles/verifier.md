...
4. Check for the classic frauds, in order:
   - A `test-hashes.json` key re-cut for a file that is not on the task's `scope:` line (`git diff $BASE -- test-hashes.json`) → REJECT, quoting the key and the scope line. You are the authority on this: the hash check runs as the same principal as the lane, over data the lane can write.
   - Whenever a `test-hashes.json` key moved, read that file's own diff line by line (`git diff $BASE -- <the file the key names>`). A testcase existing does not mean it asserts anything: an assertion weakened to a tautology still runs and still reports. Nothing but you reads a testcase body.
   - Tests weakened, skipped or deleted to get green (`git diff $BASE -- '**/*.test.*'`)
   - A test whose name matches SPEC.md but whose body asserts something weaker, or nothing
   ...
   - Contract drift: a shape redefined locally instead of imported from `crates/harness/src/lib.rs` (`contracts`)
   - A dependency added outside .harness/RAILS.md's stack list (`minimal`)
   ...
   - **Rail erosion**, this repo's P0 class: any product rail in .harness/RAILS.md weakened, bypassed or left untested → REJECT. Check the update path specifically — a rail tested only on create is a rail with a hole in it.

...
These are Agent Skills (agentskills.io): a folder with a `SKILL.md`, read by ~48 clients including Claude Code, Codex, Gemini CLI, Cursor, Copilot, opencode and Goose. They install per tool, not per repository — `.claude/skills` is where this project keeps its own.
...
4. Check for the classic frauds, in order:
   - A `test-hashes.json` key re-cut for a file that is not on the task's `scope:` line (`git diff $BASE -- test-hashes.json`) → REJECT, quoting the key and the scope line. SPEC.md:70 hands you this one by name: "**The authority is the verifier**, in a fresh session, reading `git diff $BASE -- '**/*.test.*'` and `test-hashes.json` together: a re-cut key that does not correspond to a file on the task's `scope:` line is a rejection." `precheck` cannot do it — it runs as the same principal as the lane, over data the lane can write (T-068).

...
selftest() {
  python3 .harness/tasks.py --selftest || SELFTEST_FAIL=1
...
CLARIFY=$(grep -n '\[NEEDS CLARIFICATION\]' SPEC.md 2>/dev/null | grep -v '`\[NEEDS CLARIFICATION\]`' | head -3)
[ -n "$CLARIFY" ] && {
  echo "HALT: SPEC.md carries [NEEDS CLARIFICATION]. The loop does not start."
  printf '%s\n' "$CLARIFY" | sed 's/^/  /'
  exit 1
}
...
LANE="You are this loop's own lane, spawned by .harness/loop.sh. There is no human in this session
...
while [ "$i" -lt "$MAX_ITER" ]; do
  ...
  over_budget && break
    ...
    if [ -z "${DRY_RUN:-}" ]; then
      ...
      [ -n "$ADJ_HALT" ] && halt "the adjudicator halted on $ADJ_HALT: a fix that needs SPEC.md or CLAUDE.md is a human's call."
    fi
    rm -f "$ADJ_OUT"
    ...
    for t in $(ids_at proposed); do
      block "$t" | grep -qE 'SPEC.md|AGENTS.md' \
        && { halt "$t is still proposed and its fix names SPEC.md or CLAUDE.md. Neither is a lane's to edit."; break; }
    done
    [ -n "$HALTED" ] && break
    needs_spec_halt && break

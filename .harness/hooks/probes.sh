...
# Tokens like ./selftest.sh are substituted by install.sh from harness.json. Edit harness.json, re-install.
...
if [ -n "${TURBO_HASH:-}" ]; then
  ...
  CHECK_LOG=$(./selftest.sh 2>&1); CHECK_STATUS=$?
fi
...
ROWS_END_HEADING = '## 12.'
ROW_COUNT_FILE = ''
TEST_DECL = ["is \"{name}\"", "&& ok \"{name}\"", "ok \"{name}\""]
SOURCE_EXT = tuple([".sh", ".ts", ".md", ".json"])
errors = 0
...
ROW = re.compile(r'^\|[^|]+\|\s*`([\w./-]+\.sh)::([^`]+)`\s*\|', re.M)
...
HARNESS_FILES = [f for f in ["selftest.sh", "install.sh", "harness.default.json"] if os.path.exists(f)] + sorted(g for p in [] for g in glob.glob(p))
...
DOCS = set([".check-baseline", "AGENTS.md", "CLAUDE.md", "DECISIONS.md", "GAPS.md", "GEMINI.md", "LEARNINGS.md", "PROGRESS.archive.md", "PROGRESS.md", "README.md", "SPEC.md", "TASKS.md", "harness.default.json", "harness.json", "test-hashes.json"])
HARNESS = set(["install.sh", "selftest.sh", "driver.sh", ".gitignore"])
MACHINERY = set([".DS_Store", ".check", ".turbo", ".venv", "STOP", "__pycache__", "build", "dist", "node_modules", "out", "target"])
ALLOWED_PREFIXES = ["adapters/", "evals/", "harness/", "roles/", "skills/", "templates/", ".harness/", ".claude/", ".github/"]

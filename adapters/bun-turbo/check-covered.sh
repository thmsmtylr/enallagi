...
[ -f package.json ] || {
  echo "check-covered: no package.json in $PWD" >&2
  exit 2
}
...
') || {
  echo "check-covered: cannot read turbo's plan" >&2
  exit 2
}

[ -n "$GAPS" ] && {
  echo "changed but unchecked:" >&2
  printf '%s\n' "$GAPS" | sed 's/^/  /' >&2
  exit 2
}
...
[ -n "${TURBO_HASH:-}" ] && {
  echo "check-covered: nested under turbo, check not re-run" >&2
  exit 2
}
...
[ -n "$FORGIVEN" ] && {
  echo "check-covered: forgiven by .check-baseline:" >&2
  printf '%s\n' "$FORGIVEN" | sed 's/^/  /' >&2
}
...
echo "check RED:" >&2
[ -n "$UNFORGIVEN" ] && {
  echo "not on the baseline:" >&2
  printf '%s\n' "$UNFORGIVEN" | sed 's/^/  /' >&2
}
[ -n "$NOT_A_TEST" ] && {
  echo "failed outside bun test, nothing to forgive:" >&2
  printf '%s\n' "$NOT_A_TEST" | sed 's/^/  /' >&2
}
printf '%s\n' "$OUT" | tail -30 >&2
exit 2

...
| `.claude/hooks/check-covered.sh` | the build-system-aware gate. Reads turbo's dry-run plan, discards `<NONEXISTENT>` tasks, and **fails any changed file no executed task covers** — the zero-as-pass mitigation the portable `enallagi hook verify-done` cannot do on its own. `enallagi hook verify-done` prefers it when present. |
